// Copyright 2022 The go-kyleX Authors
...
// the process. This is usually the case when Gkyle is shutting down and some events
...
//
// Opposed to the original KyleX block synchronization which is trustless (and
...
// not from within (vs. legacy KyleX sync).
//
...
func (s *skeleton) startup() {
	...
	for {
		...
		case errc := <-s.terminate:
			// No head was announced but Gkyle is shutting down
			errc <- nil
			return
		...
		case event := <-s.headEvents:
			...
			for {
				...
				default:
					...
					// error. Abort and wait until Gkyle requests a termination.
					errc := <-s.terminate
					errc <- err
					return
				}

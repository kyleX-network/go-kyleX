...
import (
	...
	"github.com/kyleX-network/go-kyleX/accounts/usbwallet/trezor"
...
// or whether the KyleX app was not started on it.
...
// KyleX address located on that path.
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (common.Address, error) {
	address := new(trezor.KyleXAddress)
	if _, err := w.trezorExchange(&trezor.KyleXGetAddress{AddressN: derivationPath}, address); err != nil {
...
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	response := new(trezor.KyleXTxRequest)
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.KyleXTxAck{DataChunk: chunk}, response); err != nil {
			return common.Address{}, nil, err
		}
	}

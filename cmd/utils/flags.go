...
// Package utils contains internal helper functions for go-kyleX commands.
package utils
...
import (
	...
	"github.com/kyleX-network/go-kyleX/common/fdlimit"
	...
	ethcatalyst "github.com/kyleX-network/go-kyleX/eth/catalyst"
	"github.com/kyleX-network/go-kyleX/eth/downloader"
	"github.com/kyleX-network/go-kyleX/eth/ethconfig"
	"github.com/kyleX-network/go-kyleX/eth/filters"
	"github.com/kyleX-network/go-kyleX/eth/gasprice"
	"github.com/kyleX-network/go-kyleX/eth/tracers"
	"github.com/kyleX-network/go-kyleX/ethdb"
	"github.com/kyleX-network/go-kyleX/ethdb/remotedb"
	"github.com/kyleX-network/go-kyleX/ethstats"
	"github.com/kyleX-network/go-kyleX/graphql"
	"github.com/kyleX-network/go-kyleX/internal/ethapi"
	"github.com/kyleX-network/go-kyleX/internal/flags"
	"github.com/kyleX-network/go-kyleX/les"
	lescatalyst "github.com/kyleX-network/go-kyleX/les/catalyst"
	"github.com/kyleX-network/go-kyleX/log"
	"github.com/kyleX-network/go-kyleX/metrics"
	"github.com/kyleX-network/go-kyleX/metrics/exp"
	"github.com/kyleX-network/go-kyleX/metrics/influxdb"
	...
	"github.com/kyleX-network/go-kyleX/p2p/nat"
	"github.com/kyleX-network/go-kyleX/p2p/netutil"
...
var (
	...
	RPCGlobalTxFeeCapFlag = &cli.Float64Flag{
		Name:     "rpc.txfeecap",
		Usage:    "Sets a cap on transaction fee (in kyle) that can be sent via the RPC APIs (0 = no cap)",
	...
	IgnoreLegacyReceiptsFlag = &cli.BoolFlag{
		...
		Usage:    "Gkyle will start up even if there are legacy receipts in freezer",
...
func MakeDataDir(ctx *cli.Context) string {
	if path := ctx.String(DataDirFlag.Name); path != "" {
		if ctx.Bool(RopstenFlag.Name) {
			// Maintain compatibility with older Gkyle configurations storing the
...
// for Gkyle and returns half of the allowance to assign to the database.
...
func MakeAddress(ks *keystore.KeyStore, account string) (accounts.Account, error) {
	...
	log.Warn("Please use explicit addresses! (can search via `gkyle account list`)")
	log.Warn("-------------------------------------------------------------------")
...
// RegisterEthService adds an KyleX client to the stack.
// The second return value is the full node instance, which may be nil if the
// node is running as a light client.
func RegisterEthService(stack *node.Node, cfg *ethconfig.Config) (ethapi.Backend, *eth.KyleX) {
	if cfg.SyncMode == downloader.LightSync {
		backend, err := les.New(stack, cfg)
		if err != nil {
			Fatalf("Failed to register the KyleX service: %v", err)
		}
...
// RegisterEthStatsService configures the KyleX Stats daemon and adds it to the node.
func RegisterEthStatsService(stack *node.Node, backend ethapi.Backend, url string) {
	if err := ethstats.New(stack, backend, backend.Engine(), url); err != nil {
		Fatalf("Failed to register the KyleX Stats service: %v", err)
	}
}
...
func SetupMetrics(ctx *cli.Context) {
	if metrics.Enabled {
		...
		if enableExport {
			...
			go influxdb.InfluxDBWithTags(metrics.DefaultRegistry, 10*time.Second, endpoint, database, username, password, "gkyle.", tagsMap)
		} else if enableExportV2 {
			tagsMap := SplitTagsFlag(ctx.String(MetricsInfluxDBTagsFlag.Name))

			log.Info("Enabling metrics export to InfluxDB (v2)")

			go influxdb.InfluxDBV2WithTags(metrics.DefaultRegistry, 10*time.Second, endpoint, token, bucket, organization, "gkyle.", tagsMap)
		}

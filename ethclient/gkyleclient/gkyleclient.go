...
// Package gkyleclient provides an RPC client for gkyle-specific APIs.
package gkyleclient
...
// Client is a wrapper around rpc.Client that implements gkyle-specific functionality.
//
// If you want to use the standardized KyleX RPC functionality, use ethclient.Client instead.
type Client struct {
	c *rpc.Client
}
...
func (ec *Client) CreateAccessList(ctx context.Context, msg kyleX.CallMsg) (*types.AccessList, uint64, string, error) {
...
func (ec *Client) CallContract(ctx context.Context, msg kyleX.CallMsg, blockNumber *big.Int, overrides *map[common.Address]OverrideAccount) ([]byte, error) {
...
// GCStats retrieves the current garbage collection stats from a gkyle node.
...
// MemStats retrieves the current memory stats from a gkyle node.
...
// GetNodeInfo retrieves the node info of a gkyle node.

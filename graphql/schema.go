...
const schema string = `
    ...
    scalar Bytes32
    # Address is a 20 byte KyleX address, represented as 0x-prefixed hexadecimal.
    scalar Address
    ...
    # Account is an KyleX account at a particular block.
    type Account {
    ...
    # Transaction is an KyleX transaction.
    type Transaction {
    ...
    type Block {
        ...
        # Account fetches an KyleX account at the current block's state.
    ...
    type Pending {
      ...
      transactions: [Transaction!]
      # Account fetches an KyleX account for the pending state.
      account(address: Address!): Account!
    ...
    type Query {
        # Block fetches an KyleX block by number or by hash. If neither is

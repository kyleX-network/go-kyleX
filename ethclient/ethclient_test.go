...
// Verify that Client implements the kyleX interfaces.
var (
	_ = kyleX.ChainReader(&Client{})
	_ = kyleX.TransactionReader(&Client{})
	_ = kyleX.ChainStateReader(&Client{})
	_ = kyleX.ChainSyncReader(&Client{})
	_ = kyleX.ContractCaller(&Client{})
	_ = kyleX.GasEstimator(&Client{})
	_ = kyleX.GasPricer(&Client{})
	_ = kyleX.LogFilterer(&Client{})
	_ = kyleX.PendingStateReader(&Client{})
	// _ = kyleX.PendingStateEventer(&Client{})
	_ = kyleX.PendingContractCaller(&Client{})
)
...
func newTestBackend(t *testing.T) (*node.Node, []*types.Block) {
	...
	// Create KyleX Service
	config := &ethconfig.Config{Genesis: genesis}
	config.Ethash.PowMode = ethash.ModeFake
	ethservice, err := eth.New(n, config)
	if err != nil {
		t.Fatalf("can't create new kyleX service: %v", err)
	}
...
func testTransactionInBlockInterrupted(t *testing.T, client *rpc.Client) {
	...
	if err == nil || err == kyleX.NotFound {
		t.Fatal("error should not be nil/notfound")
	}

	// Test tx in block not found.
	if _, err := ec.TransactionInBlock(context.Background(), block.Hash(), 20); err != kyleX.NotFound {
		t.Fatal("error should be kyleX.NotFound")
	}
}

...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of gkyle is "gkyle". If no
...
func (c *Config) NodeName() string {
	name := c.name()
	// Backwards compatibility: previous versions used title-cased "Gkyle", keep that.
	if name == "gkyle" || name == "gkyle-testnet" {
		name = "Gkyle"
	}
	if c.UserIdent != "" {
		name += "/" + c.UserIdent
...
// These resources are resolved differently for "gkyle" instances.
var isOldGkyleResource = map[string]bool{
	"chaindata":          true,
	"nodes":              true,
	"nodekey":            true,
...
func (c *Config) ResolvePath(path string) string {
	...
	// by gkyle 1.4 are used if they exist.
	if warn, isOld := isOldGkyleResource[path]; isOld {
		oldpath := ""
		if c.name() == "gkyle" {
		...
		if oldpath != "" && common.FileExist(oldpath) {
			if warn {
				c.warnOnce(&c.oldGkyleResourceWarning, "Using deprecated resource file %s, please move this file to the 'gkyle' subdirectory of datadir.", oldpath)
			}
			return oldpath
		}
	}
...
func getKeyStoreDir(conf *Config) (string, bool, error) {
	...
	if keydir == "" {
		// There is no datadir.
		keydir, err = os.MkdirTemp("", "go-kyleX-keystore")
		isEphemeral = true
	}

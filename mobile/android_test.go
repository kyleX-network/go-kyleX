...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testAccountManagement() {
		...
		KeyStore ks = new KeyStore(getInstrumentation().getContext().getFilesDir() + "/keystore", Gkyle.LightScryptN, Gkyle.LightScryptP);
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.kyleX", new NodeConfig());
			node.start();
			...
			KyleXClient ec = node.getKyleXClient();
	...
	// signatures too. Regression test for go-kyleX issue #14599.
...
func TestAndroid(t *testing.T) {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/kyleX/gkyletest", "libs"} {
		err = os.MkdirAll(dir, os.ModePerm)
		if err != nil {
			t.Fatal(err)
		}
	}
	// Generate the mobile bindings for Gkyle and add the tester class
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.kyleX", "github.com/kyleX-network/go-kyleX/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	cp.CopyFile(filepath.Join("libs", "gkyle.aar"), "gkyle.aar")

	if err = os.WriteFile(filepath.Join("src", "androidTest", "java", "org", "kyleX", "gkyletest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {

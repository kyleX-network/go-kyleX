...
import (
	...
	"github.com/kyleX-network/go-kyleX/core/beacon"
...
type ethNode struct {
	...
	ethBackend *eth.KyleX
	lapi       *lescatalyst.ConsensusAPI
	lesBackend *les.LightKyleX
}
...
func makeFullNode(genesis *core.Genesis) (*node.Node, *eth.KyleX, *ethcatalyst.ConsensusAPI, error) {
	// Define the basic configurations for the KyleX node
	...
	// Create the node and configure a full KyleX node on it
...
func makeLightNode(genesis *core.Genesis) (*node.Node, *les.LightKyleX, *lescatalyst.ConsensusAPI, error) {
	// Define the basic configurations for the KyleX node
	datadir, _ := os.MkdirTemp("", "")

// Copyright 2015 The go-kyleX Authors
...
import (
	...
	"github.com/kyleX-network/go-kyleX/accounts/abi"
	"github.com/kyleX-network/go-kyleX/accounts/keystore"
	"github.com/kyleX-network/go-kyleX/accounts/scwallet"
	"github.com/kyleX-network/go-kyleX/common"
	"github.com/kyleX-network/go-kyleX/common/hexutil"
	"github.com/kyleX-network/go-kyleX/common/math"
	"github.com/kyleX-network/go-kyleX/consensus/ethash"
	"github.com/kyleX-network/go-kyleX/consensus/misc"
	"github.com/kyleX-network/go-kyleX/core"
	"github.com/kyleX-network/go-kyleX/core/state"
	"github.com/kyleX-network/go-kyleX/core/types"
	"github.com/kyleX-network/go-kyleX/core/vm"
	"github.com/kyleX-network/go-kyleX/crypto"
	"github.com/kyleX-network/go-kyleX/eth/tracers/logger"
...
// KyleXAPI provides an API to access KyleX related information.
type KyleXAPI struct {
	b Backend
}

// NewKyleXAPI creates a new KyleX protocol API.
func NewKyleXAPI(b Backend) *KyleXAPI {
	return &KyleXAPI{b}
}

// GasPrice returns a suggestion for a gas price for legacy transactions.
func (s *KyleXAPI) GasPrice(ctx context.Context) (*hexutil.Big, error) {
...
func (s *KyleXAPI) MaxPriorityFeePerGas(ctx context.Context) (*hexutil.Big, error) {
	tipcap, err := s.b.SuggestGasTipCap(ctx)
	if err != nil {
		return nil, err
	}
...
func (s *KyleXAPI) FeeHistory(ctx context.Context, blockCount rpc.DecimalOrHex, lastBlock rpc.BlockNumber, rewardPercentiles []float64) (*feeHistoryResult, error) {
...
func (s *KyleXAPI) Syncing() (interface{}, error) {
...
// KyleXAccountAPI provides an API to access accounts managed by this node.
// It offers only methods that can retrieve accounts.
type KyleXAccountAPI struct {
	am *accounts.Manager
}

// NewKyleXAccountAPI creates a new KyleXAccountAPI.
func NewKyleXAccountAPI(am *accounts.Manager) *KyleXAccountAPI {
	return &KyleXAccountAPI{am: am}
}

// Accounts returns the collection of accounts this node manages.
func (s *KyleXAccountAPI) Accounts() []common.Address {
...
// Sign calculates an KyleX ECDSA signature for:
// keccak256("\x19KyleX Signed Message:\n" + len(message) + message))
//
...
//
// https://github.com/kyleX-network/go-kyleX/wiki/Management-APIs#personal_sign
...
// the address of:
// hash = keccak256("\x19KyleX Signed Message:\n"${message length}${message})
// addr = ecrecover(hash, signature)
//
// Note, the signature must conform to the secp256k1 curve R, S and V values, where
// the V value must be 27 or 28 for legacy reasons.
//
// https://github.com/kyleX-network/go-kyleX/wiki/Management-APIs#personal_ecRecover
func (s *PersonalAccountAPI) EcRecover(ctx context.Context, data, sig hexutil.Bytes) (common.Address, error) {
	if len(sig) != crypto.SignatureLength {
		return common.Address{}, fmt.Errorf("signature must be %d bytes long", crypto.SignatureLength)
	}
	if sig[crypto.RecoveryIDOffset] != 27 && sig[crypto.RecoveryIDOffset] != 28 {
		return common.Address{}, fmt.Errorf("invalid KyleX signature (V is not 27 or 28)")
	}
...
// Unpair deletes a pairing between wallet and gkyle.
...
// BlockChainAPI provides an API to access KyleX blockchain data.
type BlockChainAPI struct {
	b Backend
}

// NewBlockChainAPI creates a new KyleX blockchain API.
func NewBlockChainAPI(b Backend) *BlockChainAPI {
	return &BlockChainAPI{b}
}

// ChainId is the EIP-155 replay-protection chain id for the current KyleX chain config.
//
...
// See: https://github.com/kyleX-network/wiki/wiki/JSON-RPC-Error-Codes-Improvement-Proposal
...
//
// https://github.com/kyleX-network/wiki/wiki/JSON-RPC#eth_sign
...
// DebugAPI is the collection of KyleX APIs exposed over the debugging
// namespace.
...
// Version returns the current kyleX protocol version.
...
func checkTxFee(gasPrice *big.Int, gas uint64, cap float64) error {
	...
	feeEth := new(big.Float).Quo(new(big.Float).SetInt(new(big.Int).Mul(gasPrice, new(big.Int).SetUint64(gas))), new(big.Float).SetInt(big.NewInt(params.Kyle)))
	feeFloat, _ := feeEth.Float64()
	if feeFloat > cap {
		return fmt.Errorf("tx fee (%.2f kyle) exceeds the configured cap (%.2f kyle)", feeFloat, cap)
	}
	return nil
}

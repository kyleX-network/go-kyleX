...
import (
	...
	"github.com/kyleX-network/go-kyleX/accounts/external"
	"github.com/kyleX-network/go-kyleX/accounts/keystore"
	"github.com/kyleX-network/go-kyleX/accounts/scwallet"
	"github.com/kyleX-network/go-kyleX/accounts/usbwallet"
...
func loadConfig(file string, cfg *gkyleConfig) error {
	f, err := os.Open(file)
	if err != nil {
		return err
	}
...
func defaultNodeConfig() node.Config {
	...
	cfg.IPCPath = "gkyle.ipc"
	return cfg
}

// makeConfigNode loads gkyle configuration and creates a blank node instance.
func makeConfigNode(ctx *cli.Context) (*node.Node, gkyleConfig) {
	// Load defaults.
	cfg := gkyleConfig{
...
// makeFullNode loads gkyle configuration and creates the KyleX backend.
func makeFullNode(ctx *cli.Context) (*node.Node, ethapi.Backend) {
	...
	if eth != nil && !ctx.IsSet(utils.IgnoreLegacyReceiptsFlag.Name) {
		...
		if err != nil {
			...
			stack.Close()
			utils.Fatalf("Database has receipts with a legacy format. Please run `gkyle db freezer-migrate`.")
		}
	}
	...
	// Add the KyleX Stats daemon if requested.
	if cfg.Ethstats.URL != "" {
...
func applyMetricConfig(ctx *cli.Context, cfg *gkyleConfig) {

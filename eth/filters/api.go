...
// information related to the KyleX protocol such als blocks, transactions and logs.
...
func (api *FilterAPI) Logs(ctx context.Context, crit FilterCriteria) (*rpc.Subscription, error) {
	...
	logsSub, err := api.events.SubscribeLogs(kyleX.FilterQuery(crit), matchedLogs)
	if err != nil {
		return nil, err
	}
...
// Same as kyleX.FilterQuery but with UnmarshalJSON() method.
type FilterCriteria kyleX.FilterQuery
...
func (api *FilterAPI) NewFilter(crit FilterCriteria) (rpc.ID, error) {
	...
	logsSub, err := api.events.SubscribeLogs(kyleX.FilterQuery(crit), logs)
	if err != nil {
		return "", err
	}
